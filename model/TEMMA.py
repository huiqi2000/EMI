...
class TEmbeddings(nn.Module):
    def __init__(self, opts, dim):
        ...
        self.levels = opts.levels  # 5
        self.ksize = opts.ksize
        self.d_model = opts.d_model
        self.dropout = opts.dropout

        self.channel_sizes = [self.d_model] * self.levels   #[128, 128, 128, 128, 128]
...
class TE(nn.Module):       #TE(args, 1024)

    def __init__(self, opts, num_features):
        super(TE, self).__init__()

        self.modal_num = 1   #表示模态的个数
        assert self.modal_num == 1, 'TE model is only used for single feature streams ...'

        # self.mask_a_length = int(opts.mask_a_length)  #50
        # self.mask_b_length = int(opts.mask_b_length)  #10

        self.N = opts.block_num     #4
        self.dropout = opts.dropout   #0.2
        self.h = opts.h    #4
        self.d_model = opts.d_model   #128
        self.d_ff = opts.d_ff   #256
        ...
        #     nn.Linear(self.d_model*self.seqlen, self.d_model*self.seqlen // 2),
        #     nn.ReLU(),
        #     nn.Linear(self.d_model*self.seqlen // 2, opts.ntarget)
        # )
...
class TEMMA(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.input_v = ProcessInput(opts, 384)
        self.input_a = ProcessInput(opts, 768)
        ...
        for p in self.input_a.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)

        for p in self.input_v.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)
    ...
    def forward(self, x_v, x_a):
        ...
        return self.regress(x).mean(dim=1)
...
if __name__ == '__main__':
    ...
    x_v = torch.randn(4, 100, 384)
    # x_a = torch.randn(4, 300, 768)
